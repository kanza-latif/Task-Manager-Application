...
CREATE TABLE
    IF NOT EXISTS `whitelist_table` (
        `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
        `msisdn` VARCHAR(20) NOT NULL
    );

ALTER TABLE `whitelist_table` ADD UNIQUE `whitelist_table_msisdn_unique` (`msisdn`);
...
ALTER TABLE `user_table` ADD UNIQUE `user_table_username_unique` (`username`);

ALTER TABLE `user_table` ADD UNIQUE `user_table_email_unique` (`email`);

CREATE TABLE
    IF NOT EXISTS `session_table` (
...
ALTER TABLE `alarm_table` ADD INDEX `alarm_table_time_raised_index` (`time_raised`);

ALTER TABLE `alarm_table` ADD CONSTRAINT `alarm_table_updated_by_foreign` FOREIGN KEY (`updated_by`) REFERENCES `user_table` (`id`);